...
export default {
  ...
  methods: {
    getOrder() {
      var self = this;
      axios.post('http://localhost:9091/hvs/order/getOrderAndFileByOrderid',{
    ...
    getApplicant() {
      var self = this;
      axios.post('http://localhost:9091/hvs/order/getApplicant',{
        orderid: self.orderid
      }).then(function(res){
    ...
    getBidder() {
      var self = this;
      axios.post('http://localhost:9091/hvs/order/getBidder',{
        orderid: self.orderid
      }).then(function(res){
    ...
    getOfferByOrderid() {
      var self = this;
      axios.post('http://localhost:9091/hvs/offer/getOfferByOrderid',{
        orderid: self.orderid
      }).then(function(res){
    ...
    addBidding() {
      ...
      if (this.money >= this.orderForm.minmoney) {
        ...
        this.$confirm('报价高于最低价，你可能竞拍失败，还要继续报价吗？', '提示', {
          ...
          axios.post('http://localhost:9091/hvs/offer/addOffer',{
            orderid: self.orderid,
    ...
    isMyOrder() {
      var self = this
      axios.post('http://localhost:9091/hvs/order/isMyOrder',{
        orderid: self.orderid,
        userid: self.$store.state.user.userid
      }).then(function(res){
        if (res.data.success) {
